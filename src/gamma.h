...
// see https://hal.archives-ouvertes.fr/hal-01329669v2/document
// https://hal.archives-ouvertes.fr/hal-01329669v2
...
constexpr const double DPMIN = 1.0e-300; /* Number near the smallest representable double-point number */
constexpr const int ITMAX = 1000000000; /* Maximum allowed number of iterations */
constexpr const int NITERMAX_ROMBERG = 15; /* Maximum allowed number of Romberg iterations */
constexpr const double TOL_ROMBERG = 0.1; /* Tolerance factor used to stop the Romberg iterations */
constexpr const double GAMMAEPS = 2.22e-16; /* Machine GAMMAEPSilon */
constexpr const double TOL_DIFF = 0.2; /* Tolerance factor used for the approximation of I_{x,y}^{mu,p} using differences */


static double plim(const double & x)
{
...
void G_cfrac_lower(double *Gcfrac, double p, double x)
{
  ...
  while((fabs(del-1.0) >= GAMMAEPS) && (n < ITMAX));
  *Gcfrac = f;
...
static void romberg_iterations(double *R, double sigma,int n,double x,double y,double mu,double p,double h,double pow2)
{
...
void romberg_estimate(double *rho,double *sigma,double x,double y,double mu,double p)
{
  int n,adr0;
  ...
  *sigma = -mu*y + (p-1)*log(y);
  ...
  /* loop for n > 0 */
  relneeded = GAMMAEPS/TOL_ROMBERG;
  adr0 = 0;
  n = 1;
...
double lowgamma(const double & p, const double & x)
{
    double G = 0.0;
    if (fabs(x) == 0.0 || x < 0) return G;
    G_func(&G, p, x);
    if (x <= p) return G * exp(-x + p * log(x)); // equation 24 in https://hal.archives-ouvertes.fr/hal-01329669v2/document
    else return tgamma(p) - G * exp(-x + p * log(x)); // equation 25 in https://hal.archives-ouvertes.fr/hal-01329669v2/document
}