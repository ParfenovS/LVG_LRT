...
class RT_time : public RT_time_base		// integrates kinetic equations for level populations over time; the non-linear system of equations at each time step is solved with fixed point iterations
{
private:

	double populate_matrix_vector(double A[], double B[], beta_LVG & LVG_beta, const vector <vector <double> > & oldpops, const double BDF_coeffs[], const double & h, double dpop_dt[])	// fill the matrix A and vector B from the non-linear system of equations A*X=B which is obtained at each step of integration over time; compute vector of derivatives of populations over time and its norm
	{
		...
		for (size_t i = 0; i < n; i++) {
			...
			B[i] = BDF_coeffs[1]* oldpops[i][0] + BDF_coeffs[2] * oldpops[i][1];		// BDF method formula
...
public:
	...
	int radiative_transfer() override		// main function which should be called to perform radiative transfer calculations
	{
		...
		do {
			...
			do {							// solve non-linear system of equations at a given time step with fixed point iterations
				F_norm = populate_matrix_vector(A, pop, LVG_beta, oldpops_time, BDF_coeffs, h, dpop_dt);
			...
			if (monitor_function > MON_FUN_UPPER_LIMIT) {	// one need to roll-back populations and repeat integration with a smaller time step
				if (h > MIN_TIME_STEP) {
					...
					clear_mem_close_files(A, pop, dpop_dt, oldpops_Ng, oldpops_time, binpopfile, binTbrfile, binTexfile, bintaufile);
	...
	RT_time() noexcept : RT_time_base()
	{}

	RT_time(istream & cin) : RT_time_base(cin)
	{}

	RT_time(const unsigned short & initialSolutionSource, const double & MAX_POPS_EPS, const unsigned long & maxNumberOfIterations, const double & beamH, const double & lineWidth) : RT_time_base(initialSolutionSource, MAX_POPS_EPS, maxNumberOfIterations, beamH, lineWidth)
	{}
};
